  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white">
      ...
      <header className="flex items-center justify-between px-6 py-5 border-b border-white/10">
        <a href="/" className="text-white text-sm uppercase tracking-widest font-bold">
          Vibe
        </a>
        <button
                  ...
                  {p.interests.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {p.interests.slice(0, 3).map((tag) => (
                        <span key={tag} className="text-xs px-2 py-0.5 bg-white/10 text-white/50">
                          {tag}
                        </span>
                      ))}
                      {p.interests.length > 3 && (
                        <span className="text-xs px-2 py-0.5 text-white/25">
                          +{p.interests.length - 3}
                        </span>
                      )}
                    </div>
                  )}
                  <div className="mt-4 flex items-center gap-1 text-white/20 group-hover:text-white/50 transition-colors text-xs uppercase tracking-wide">
                    <span>Открыть</span>
                ...
                <>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selected.interests.map((tag) => (
                      <span key={tag} className="text-xs px-3 py-1 bg-white/10 text-white/60">
                        {tag}
                      </span>
                    ))}
                  </div>
